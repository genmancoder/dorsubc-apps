...
export function DisplayArea() {
    ...
    useEffect(() => {
        ...
        if (savedDisplayCount) {
            setDisplayCount(savedDisplayCount === "all" ? "all" : Number(savedDisplayCount))
        }
        if (savedLayout === "grid" || savedLayout === "row" || savedLayout === "column") {
            ...
            {settingsOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-white p-6 rounded-lg w-80">
                        ...
                        <div className="mb-4">
                            <label className="block font-medium mb-1">Number of windows:</label>
                            <select
                                value={displayCount}
                                onChange={(e) =>
                                    setDisplayCount(
                            ...
                            <div className="flex flex-wrap gap-2">
                                {windows.map((w) => (
                                    <button
                                        key={w.id}
                                        onClick={() => {
                                            const newPinned = displaySettings.pinnedWindows.includes(w.id)
                                                ? displaySettings.pinnedWindows.filter((id) => id !== w.id)
                                                : [...displaySettings.pinnedWindows, w.id];
                                            setDisplaySettings({ ...displaySettings, pinnedWindows: newPinned });
                                        }}
                                        className={`px-3 py-1 rounded border ${displaySettings.pinnedWindows.includes(w.id)
                                ...
                                {windows.map((w) => (
                                    <label key={w.id} className="flex items-center gap-1">
                                        <input
                                            ...
                                            onChange={() => {
                                                const newVisible = displaySettings.visibleWindows.includes(w.id)
                                                    ? displaySettings.visibleWindows.filter((id) => id !== w.id)
                                                    : [...displaySettings.visibleWindows, w.id];
                                                setDisplaySettings({ ...displaySettings, visibleWindows: newVisible });
                                            }}
                                        />
                                        {w.windowTitle}
                                    </label>
            ...
            {windows.length > 0 ? (
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-min">
                    {windows
                        ...
                        .sort((a, b) => {
                            const aPinned = displaySettings.pinnedWindows.includes(a.id) ? -1 : 0;
                            const bPinned = displaySettings.pinnedWindows.includes(b.id) ? -1 : 0;
